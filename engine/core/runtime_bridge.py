...
class RuntimeBridge:
    ...
    def prepare(self) -> RuntimePaths:
        """Create required folders and prepend portable folders to sys.path."""
        for folder in (
            self.paths.libs_dir,
        ...
        portable_paths = unique_existing_paths(
            [self.paths.libs_dir, self.paths.scripts_dir, self.paths.embedded_python_dir]
        )
